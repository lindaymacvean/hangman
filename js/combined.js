if(typeof Ehangman === 'undefined') { //stop collisions
	try {
		var window.Ehangman = function() {
			...
			//1. SET theWord
			//if the words array is available offer the user a prompt, if prompt is not set get theWord from words array defined in words.js... 
			if (words) {
				theWord = prompt("Please enter your word, or just hit return for a random word");
				if(typeof theWord !== 'string') { 
				//if theWord has not been set by prompt
					theWord = function() {
						var a = 0;
						var i = 0
						for (each in words) { 
		...
		}(); //self invoke Ehangman and return processLettre as a method
